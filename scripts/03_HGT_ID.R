...
# returns a table containing only genes that are present in the result and the donor
# will not return a gene if it is present in recipient genome
# base_dir = directory containing roary results
# donor =  string, exact name for donor genome (no file suffix)
# recipient = string, exact name for recipient genome (no file suffix)
# result =  string, exact name for result genome (no file sufix)
generate_transferred_gene_table <- 
  function(base_dir){
...
HGT_ID <- 
  function(base_dir, clust_level){
    # this selects gifrop identified genomic islands (by specified clust level)
    # that are present in donor and the result.
    # (they may also be present in the recipient)
    # base_dir = directory containing roary results
    # clust level must be a string that exactlt matches the column for the 
      # clsutering level you wan to use
    
    # clust_level <- enquo(clust_level)
    # this next line is not generalized.
    genomes <- unlist(strsplit(base_dir, split='/'))[4]
    genomes <- unlist(strsplit(genomes, split = '_'))
...
plot_islands <- function(HGT_ID_result, clust_level){
  ...
  if (nrow(HGT_ID_result) < 1){
    return('nothing to plot')
  }
  clust_level <- enquo(clust_level)
  
  HGT_ID_result %>% 
    select(genome_name, start, end, !!clust_level, HGT_role, seqid_len) %>% 
    arrange(!!clust_level) %>% 
    ...
    ggtitle('Genomic Island locations in donor and result genomes')
}
...
# genes present in recipient but not result
DELETED <- lapply(these_dirs, generate_deleted_gene_table)
...
# results12 <- RESULTS[[12]]
# results13 <- RESULTS[[13]]
# 
# test <- 
#   results12 %>% group_by(seqid) %>% 
#   nest() 


# 
# test1 <- combn(test$data[[2]]$island_ID, 2)
# 
# 
# expand_grid()
# ###
...
# base_dir <- './outputs/pan_genomes/14229-5_6461_SampleA/pan/'
# 
# split_11601MDx6631 <-
