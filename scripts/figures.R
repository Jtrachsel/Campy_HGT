...
gpa_abric <- gpa %>%
  ...
  left_join(abricate) %>% select(Gene, Annotation,`Genome Fragment`, PRODUCT, RESISTANCE) %>% 
  na.omit()
...
gpa_long$RESISTANCE <- factor(gpa_long$RESISTANCE)
...
gpa_long %>% 
  ...
  geom_point(data=filter(gpa_long, !is.na(RESISTANCE) &(`Order within Fragment` > 435 & `Order within Fragment` < 525)),aes(color=RESISTANCE), size=2,na.rm = TRUE , shape=17)+
  geom_text(data=annot, aes(x=`Order within Fragment`, y=-6, label=Annotation2), inherit.aes = FALSE, hjust='left', size=3) + 
