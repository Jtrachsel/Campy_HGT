...
source("./03_HGT_ID.R", local = knitr::knit_global())
...
ani_tab <-
  read_tsv('/home/Julian.Trachsel/Vanina/outputs/pyani_res/ANIm_percentage_identity.tab') %>% 
...
read_tsv('../outputs/01_metadata.tsv') %>%
  select(donor_genome, recipient_genome, result_genome, -pan_dir) %>% 
  kable() %>% 
...
For each experiment there will be the following items:  
  1) Table of 'genomic islands'   
  2) Table of transferred genes  
  3) Table of deleted genes  
  4) plot showing where genomic islands are found in the donor and result genomes.  
  
The tables in this report only contain a subset of the columns for viewing ease. Full versions of these tables are available in the results directory I will provide you.  
  
  
  
### `r exp_names[1]`  

6631 X 11601MD = 11601MDx6631  


```{r, fig.width=10,fig.height=4}


RESULTS[[1]] %>% select(c(1,6, 12, 17, 18, 19, 24)) %>% 
